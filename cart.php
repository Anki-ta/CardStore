...
<div id="wrapper">
<div class="heads" >
<p class="acc_n_crt" align=right><span class="glyphicon glyphicon-user">&nbsp<a href="myaccount.php">My Account</a>&nbsp|&nbsp<span class="glyphicon glyphicon-shopping-cart">&nbsp<a href="\CardStore\cart.php">My Cart</a>&nbsp|&nbsp<span class="glyphicon glyphicon-off">&nbsp<a href="\CardStore\logout.php">Logout</a>&nbsp&nbsp</p>
...
<?php $count=mysqli_query($connect,"SELECT  count(*) FROM `shoppingcart` WHERE `custID`=".$_SESSION['CustID']);
$chckcount=mysqli_fetch_array($count); 
if($chckcount['0']==0){?>
<br></br>
<br>
...
<?php  $c=mysqli_query($connect,"SELECT  `CartID`,`pID`, `quantity`, `pprType`, `total`FROM `shoppingcart` WHERE `custID`='{$_SESSION['CustID']}'"); 
	   while($row=mysqli_fetch_array($c) )
{$_SESSION['cartID']=$row['CartID'];
/*$_SESSION['rno']['cartID']=$row['CartID'];
$_SESSION['rno']['pprType']=$row['pprType'];
$_SESSION['rno']['qty']= $row['quantity'];
$_SESSION['rno']['tot']= $row['total'];
$_SESSION['rno']['pid']= $row['pID'];*/?>
...
<td align=center><a class="btn  btn-danger" href="remove.php"  style="height:30px;width:80px;background-color:#ec1d17;
"onMouseover="this.style.background = '#d9534f'" onMouseout="this.style.background = '#ec1d17'" >remove</a>
...
<?php  }$recount=mysqli_query($connect,"SELECT  count(*) FROM `shoppingcart` WHERE `custID`=".$_SESSION['CustID']); 
$rw=mysqli_fetch_array($recount);
if($rw==0)
	$status="FALSE";
