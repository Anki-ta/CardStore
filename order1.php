...
$c=mysqli_query($connect,"SELECT  * FROM `shoppingcart` WHERE `custID`=".$_SESSION['CustID']."AND `CartID`=".$_SESSION['cartID']);
...
while($row=mysqli_fetch_array($c) )
{
	...
	$save1="UPDATE `custorder` SET `delAddr`=[' $d_addr'] WHERE `custID`=".$_SESSION['CustID']."AND `CartID`=".$_SESSION['cartID'];
